...
fun GridScreen(
    bucketId: Long?,
    onNavigateToViewer: (mediaId: String, source: MediaSource, bucketId: Long?) -> Unit,
    onBack: () -> Unit,
        ...
        topBar = {
            if (selectionState.selectionMode) {
                SelectionToolbar(
                    ...
                    onSelectAll = { selectionState.selectAll(mediaItems) },
                    onShareSelected = {
                        val selected = selectionState.getSelectedItems(mediaItems)
                ...
                TopAppBar(
                    title = { Text(if (bucketId == null) stringResource(R.string.tab_all_media) else mediaItems.firstOrNull()?.bucketName?.ifBlank { "Album" } ?: "Album") },
                    navigationIcon = {
